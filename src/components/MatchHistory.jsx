          ...
          <div key={match.id} className="px-4 py-4">
            ...
            {match.type === 'doubles' ? (
              <div className="space-y-1">
                <div className={`font-bold text-base ${match.winnerTeam==='A' ? 'text-white' : 'text-slate-500'}`}>
                  🔵 {match.teamANames?.join(' & ')} {match.winnerTeam==='A' && '🏆'}
                </div>
                <div className="text-slate-600 text-sm pl-1">vs</div>
                <div className={`font-bold text-base ${match.winnerTeam==='B' ? 'text-white' : 'text-slate-500'}`}>
                  🔴 {match.teamBNames?.join(' & ')} {match.winnerTeam==='B' && '🏆'}
                </div>
                <div className="text-green-400 text-sm mt-1">✓ {match.winnerNames?.join(' & ')} won · +10 pts each</div>
              </div>
            ) : (
              <div>
                ...
                <div className="text-green-400 text-sm mt-1">✓ {match.winnerName} won · +10 pts</div>
              </div>
            )}
          </div>
        ))}
